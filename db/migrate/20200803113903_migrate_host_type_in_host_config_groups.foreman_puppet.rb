class MigrateHostTypeInHostConfigGroups < ActiveRecord::Migration[6.0]
  class FakeHostPuppetFacet < ApplicationRecord
  ...
  def up
    ...
    host_config_group_ids.each do |host_id|
      host_facet_id = FakeHostPuppetFacet.where(host_id: host_id).pick(:id)
      FakeHostConfigGroup.where(host_type: %w[Host::Base Host::Managed], host_id: host_id).update_all(host_type: 'ForemanPuppet::HostPuppetFacet', host_id: host_facet_id)
    end
    ...
    hostgroup_config_group_ids.each do |hostgroup_id|
      ...
      FakeHostConfigGroup.where(host_type: 'Hostgroup', host_id: hostgroup_id).update_all(host_type: 'ForemanPuppet::HostgroupPuppetFacet', host_id: hostgroup_facet_id)
    end
  end

  def down
    host_config_group_ids = FakeHostConfigGroup.where(host_type: 'ForemanPuppet::HostPuppetFacet').pluck(:host_id).uniq
    host_facet_ids = FakeHostPuppetFacet.where(id: host_config_group_ids).pluck(:id, :host_id).to_h
    ...
    hostgroup_config_group_ids.each do |hostgroup_facet_id|
      ForemanPuppet::HostConfigGroup.where(host_type: 'ForemanPuppet::HostgroupPuppetFacet', host_id: hostgroup_facet_id)
